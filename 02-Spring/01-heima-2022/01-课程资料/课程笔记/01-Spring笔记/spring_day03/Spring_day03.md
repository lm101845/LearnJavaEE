...
@Aspect
public class MyAdvice {
    @Pointcut("execution(void BookDao.update1())")
    private void pt(){}
...
```java
execution(void BookDao.update())
```

描述方式二：执行com.itheima.dao.impl包下的BookDaoImpl类中的无参数update方法

```
execution(void BookDaoImpl.update())
```
...
public class MyAdvice {
    ...
    @Pointcut("execution(int BookDao.select())")
    private void pt2(){}
...
Exception in thread "main" org.springframework.aop.AopInvocationException: ==Null return value from advice does not match primitive return type for: public abstract int BookDao.select()==
...
- 编写通知类
  ...
  @Aspect
  public class MyAdvice {
      @Pointcut("execution(* BookDao.findName(..))")
      private void pt(){}
