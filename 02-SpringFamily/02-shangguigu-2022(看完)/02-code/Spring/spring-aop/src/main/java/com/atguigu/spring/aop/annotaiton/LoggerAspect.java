...
public class LoggerAspect {
    //切入点表达式的重用(弄一个公共的切入点表达式)
    @Pointcut("execution(* com.atguigu.spring.aop.annotaiton.CalculatorImpl.*(..))")
    public void pointCut(){}

    //@Before("execution(public int com.atguigu.spring.aop.annotaiton.CalculatorImpl.add(int,int))")
    //这个切入点表达式写死了,只能作用于add方法
    //@Before("execution(* com.atguigu.spring.aop.annotaiton.CalculatorImpl.*(..))")
    //这样写表示当前包下所有的类
    //@Before("execution(* com.atguigu.spring.aop.annotaiton.*.*(..))")
    @Before("pointCut()")
